...
public class EmailService {
    ...
    public CompletableFuture<Boolean> sendVerificationCode(String to, String code) {
        try {
            ...
            helper.setTo(to);
            helper.setSubject("Movie Review 이메일 인증 코드");

            String htmlContent = """
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
                        <h2 style="color: #333; border-bottom: 1px solid #e0e0e0; padding-bottom: 10px;">이메일 인증 코드</h2>
                        <p>안녕하세요,</p>
                        <p>Movie Review 회원가입을 위한 인증 코드입니다:</p>
                        ...
                        <p>회원가입 요청을 하지 않으셨다면, 이 이메일을 무시하셔도 됩니다.</p>
    ...
    /**
     * 비동기로 비밀번호 재설정 인증 코드를 이메일로 전송합니다.
     * @param to 수신자 이메일
     * @param code 인증 코드
     * @return 작업 완료를 알리는 CompletableFuture
     */
